...
interface BlockedDate {
  date: string;
  status: string; // 'blocked' atau 'expired'
}

// --- FIX parseDate supaya handle semua format date
...
export default function RoomDetailView({ roomId, isModal = false }: RoomDetailViewProps) {
  ...
  useEffect(() => {
    const fetchBlocked = async () => {
      try {
        const res = await api.get(`/room-date/${roomId}`);
        setBlockedDates(res.data); 
      } catch (err) {
        console.error("Gagal load blocked dates", err);
      }
    };
    if (roomId) fetchBlocked();
  }, [roomId]);
  ...
  const isDateUnavailable = (date: Date) => {
    ...
    const booked = existingBookings.some(b => {
      ...
      const end = parseDate(b.checkOut).getTime() + 24*60*60*1000; // hari terakhir ikut terisi
      return targetTime >= start && targetTime < end;
    });

    // blocked
    const blocked = blockedDates.some(b => {
      const bDate = parseDate(b.date).toISOString().split("T")[0];
      return bDate === target.toISOString().split("T")[0] && b.status === "blocked";
    });

    // passed
    const passed = targetTime < new Date(new Date().setHours(0,0,0,0)).getTime();

    return { booked, blocked, passed };
  };
  ...
  const isRangeAvailable = (start: string, end: string) => {
    ...
    while (curr < last) {
      const { booked, blocked } = isDateUnavailable(curr);
  ...
  const days = getDays();
  const checkLogin = () => !!localStorage.getItem("ministay_user");
  ...
  const handleMainAction = async () => {
    ...
    if (!isRangeAvailable(checkIn, checkOut)) return alert("Tanggal sudah terisi atau diblokir.");
    try {
      setIsLoading(true);
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const { booked, blocked, passed } = isDateUnavailable(dateObj);
      const isSelected = checkIn === dateStr || checkOut === dateStr;
      const isInRange = checkIn && checkOut && dateStr > checkIn && dateStr < checkOut;

      let disabledClass = "";
      if (booked) disabledClass = "text-red-600 bg-red-100 line-through cursor-not-allowed";
      else if (blocked) disabledClass = "text-yellow-700 bg-yellow-100 line-through cursor-not-allowed";
      else if (passed) disabledClass = "text-gray-300 cursor-not-allowed line-through bg-gray-50";

      daysArray.push(
        <button
          key={day}
          disabled={booked || blocked || passed}
          onClick={() => {
            if (booked || blocked || passed) return;
          ...
          title={booked ? "Sudah dipesan" : blocked ? "Diblokir admin" : passed ? "Tanggal lampau" : ""}
          className={`
            ...
            ${isInRange && !(booked || blocked || passed) ? "bg-blue-50 text-blue-600" : ""}
          `}
        >
          {day}
        </button>
        ...
        {images.length > 1 && (
          <div className="absolute bottom-3 right-3 flex gap-2 z-10">
            {images.map((img, idx) => (
              ...
              >
                <Image src={img} alt={`Thumbnail ${idx + 1}`} width={48} height={48} className="object-cover w-full h-full" />
              </button>
            ))}
          </div>
        )}
